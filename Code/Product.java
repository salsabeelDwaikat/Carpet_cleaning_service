...
public class Product {
	...
	public static boolean updatedinfromation(int i, int upd, String updated) {
		boolean statement=false;
				...
				if(upd==1) {
					prodArr.get(upd).setProdType(updated);
					statement=true;
					System.out.println("updated product information are\n"   +prodArr.get(upd));
				}
				else if(upd==2) {
					prodArr.get(upd).setProdMat(updated);
					statement=true;
					System.out.println("updated product information are\n"+prodArr.get(upd));
				}
				else if(upd==3) {
					prodArr.get(upd).setHeight(updated);
					statement=true;
					System.out.println("updated product information are\n"   +prodArr.get(upd));
				}
				else if(upd==4) {
					prodArr.get(upd).setWidth(updated);
					statement=true;
					System.out.println("updated product information are\n    "+prodArr.get(upd));
				}
				else if(upd==5) {
					prodArr.get(upd).setSpecialTreat(updated);
					statement=true;
					System.out.println("updated product information are\n   "+prodArr.get(upd));
				}
